...
// 将token设置到ajax的请求头中，在发送ajax请求的时候，将这个token的内容带过去 发送ajax请求的时候触发
$.ajaxSetup({
    // 在发送请求send之前，将token发送到服务器，让服务器验证token
    beforeSend(xhr){
        xhr.setRequestHeader('Authorization', window.localStorage.getItem('token'));
    },
    // 请求失败的时候
    error(xhr,status,error){
        // console.log(error);//结果：Forbidden
        if(error==='Forbidden'){
            // 如果验证不通过，提示用户登录，并跳转到登陆页面让用户登陆
            alert('您还没有登陆，请先登录');
